...
type ChatQuery struct {
	...
	withChatMessage *ChatMessageQuery
...
func (cq *ChatQuery) QueryChatMessage() *ChatMessageQuery {
	query := (&ChatMessageClient{config: cq.config}).Query()
...
func (cq *ChatQuery) WithChatMessage(opts ...func(*ChatMessageQuery)) *ChatQuery {
	query := (&ChatMessageClient{config: cq.config}).Query()
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withChatMessage; query != nil {
		if err := cq.loadChatMessage(ctx, query, nodes,
			func(n *Chat) { n.Edges.ChatMessage = []*ChatMessage{} },
			func(n *Chat, e *ChatMessage) { n.Edges.ChatMessage = append(n.Edges.ChatMessage, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadChatMessage(ctx context.Context, query *ChatMessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *ChatMessage)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_chat_message" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_chat_message" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
