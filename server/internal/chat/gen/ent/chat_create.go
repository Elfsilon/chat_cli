...
func (cc *ChatCreate) SetUsers(i []int) *ChatCreate {
	cc.mutation.SetUsers(i)
	return cc
}
...
func (cc *ChatCreate) AddChatMessage(c ...*ChatMessage) *ChatCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
func (cc *ChatCreate) check() error {
	...
	if _, ok := cc.mutation.Users(); !ok {
		return &ValidationError{Name: "users", err: errors.New(`ent: missing required field "Chat.users"`)}
	}
	return nil
}
...
func (cc *ChatCreate) createSpec() (*Chat, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.Users(); ok {
