...
func (cmq *ChatMessageQuery) QueryOwner() *ChatQuery {
	query := (&ChatClient{config: cmq.config}).Query()
...
func (cmq *ChatMessageQuery) WithOwner(opts ...func(*ChatQuery)) *ChatMessageQuery {
	query := (&ChatClient{config: cmq.config}).Query()
...
func (cmq *ChatMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChatMessage, error) {
	...
	if query := cmq.withOwner; query != nil {
		if err := cmq.loadOwner(ctx, query, nodes, nil,
			func(n *ChatMessage, e *Chat) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cmq *ChatMessageQuery) loadOwner(ctx context.Context, query *ChatQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *Chat)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chat_chat_message" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
